...
Scenario: Ação do botão "Permitir" no pop-up "Conhecer a sua localização"
    ...
    Then a localização deve ser exibida na cor verde no canto superior direito da página no desktop
    And a localização deve ser exibida na cor verde no menu de navegação no canto superior direito da página no dispositivo móvel

Scenario: Ação ao clicar na localização exibida no canto superior direito após permitir o acesso
    Given que permiti o acesso à localização e ela é exibida no canto superior direito
...
Scenario: Ação ao clicar em "Bloquear" e "Ok" no pop-up "Conhecer a sua localização"
    ...
    Then a localização não deve ser exibida no canto superior direito
    And um botão na cor verde, com um ícone de mapa e o texto "Obter Localização" em branco deve ser exibido no canto superior direito
...
Scenario: Ação ao clicar no botão "Obter Localização" após bloquear a localização
    ...
    When clico no botão "Obter Localização" no canto superior direito
